;(function(){
    ...
    opt.announce = function(to){
		...
		root.on('create', function(at){
			console.log('webrtc create')
			this.to.next(at);
			setTimeout(opt.announce, 1);
		});
		root.on('in', function(msg){
			// console.log('webrct in')
			// console.log(msg.rtc);
			if(msg.rtc){ open(msg) }
			this.to.next(msg);
		});
		...
		function open(msg){
			...
			console.log('webrct open')
			var rtc = msg.rtc, peer, tmp;
			// console.log('webrtc my id :' +rtc.id);
			...
			wire.onopen = function(e){
				//delete open[rtc.id];
				// console.log('webrtc에서 hiii ' + peer );
				mesh.hi(peer);
			}
			wire.onmessage = function(msg){
				// console.log('webrtc on messageeeee')
				if(!msg){ return }
        ...
        root.on('out', {'@': msg['#'], rtc: {candidate: e.candidate, id: opt.pid}});
			...
			peer.ondatachannel = function(e){
				console.log('webrtc ondatachannel')
				console.log('my webrtc id: ' + opt.id )
				var rc = e.channel;
