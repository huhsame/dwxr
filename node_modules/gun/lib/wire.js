var Gun = require('../gun');
let dAdapter = require('../../../src/server/d-adapter');
console.log('[gun/wire.js] loaded')
...
Gun.on('opt', function(root){
	...
	if(ws.server && !ws.web){
		...
		ws.web.on('connection', function(wire){ var peer;
			...
			// console.log("wire.url: "+wire.url);
			peer = {wire: wire}
			opt.mesh.hi(peer);

			dAdapter.emitConnection( peer );

			wire.on('message', function(msg){
				opt.mesh.hear(msg.data || msg, peer);
			});
			wire.on('close', function(){
				dAdapter.emitClose(peer.id);
				opt.mesh.bye(peer);
			});
