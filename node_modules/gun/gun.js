...
// mesh(이벤트 -> d-adapter(소켓으로 전달) -> 여기서 이벤트로
let dAdapter = function(){};
dAdapter.hear = function(d){
	if(!d) {return}
	try{
	...
	;USE(function(module){
		...
		function Mesh(root){
			...
			mesh.hi = function(peer){
				...
				// console.log(peer); // 여기서 피어는 .. 릴레이서버
				if(peer.id){
			...
			root.on('create', function(root){
				...
				console.log('create : '+root.opt.pid);
				this.to.next(root);
		...
		Gun.on('opt', function(root){
			...
			function open(peer){ try{
				...
				wire.onopen = function(){
					opt.mesh.hi(peer); // 여기도 내가 안찍히고.. 서버가 찍히잖아.
					console.log(peer);
				}
