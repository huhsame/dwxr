...
// for initalization
// dAdatter.emitConnection 에서 클라이언트로 데이터 정보 보내는 것 뿐아니라
// 서버한테도 피어를 보내줘서 서버에서 리스트를 가지고 있있어야 함 ==> peer-manager

// for update
// bridge.emitConnection [wire.js] ----socket:data(user:pid) to clients ---> wire.onmessage [gun.js] ----data---> d-adapter(gun.js) ----event: onuser---> experiments.js
let bridge = function(){};
bridge.hear = function(d){
	if(!d) {return}
	try{
		let data = JSON.parse(d);
		// 새로운 유저가 입장했다는 사실을 서버로부터 각 클라이언트가 받음
		// onuser event 발사 for realtime update
		if(data.type === 'user'){
			switch(data.type){
				case 'user':
					let onUser = new CustomEvent('onuser', {detail: data});
					window.dispatchEvent(onUser);
					break;
				default:
			}
		}
	...
	;USE(function(module){
		...
		function Mesh(root){
			...
			mesh.hi = function(peer){
				console.log('[gun/mesh.hi] opt.pid : ' + opt.pid)
				var tmp = peer.wire || {};
				if(peer.id){
					opt.peers[peer.url || peer.id] = peer;
				} else {
					tmp = peer.id = peer.id || Type.text.random(9);
					console.log('[gun/mesh.hi] tmp: ' + tmp)
